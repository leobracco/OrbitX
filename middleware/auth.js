...
async function required(req, res, next) {
  ...
  // Si viene X-Device-ID sin JWT → es un agente de tractor
  // La validación del token individual la hace deviceAuth en routes/devices.js
  // Acá solo lo dejamos pasar con un user de tipo device
  if (!token && req.headers["x-device-id"]) {
    req.user = {
      uid:        req.headers["x-device-id"],
      rol:        "device",
      rol_global: "device",
      estabSlug:  req.headers["x-estab-slug"] || "unassigned",
      memberships: [],
      isDevice:   true
    };
    return next();
  ...
  try {
    ...
    req.user = {
      uid:         payload.uid,
      rol_global:  payload.rol_global || payload.rol || "user",
