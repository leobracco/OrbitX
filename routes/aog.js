// routes/aog.js — API Piloto AP: recepción, historial y restauración
...
router.post("/sync", deviceAuth, async (req, res) => {
  ...
  try {
    ...
    console.log(`[PilotoAP] ✓ ${deviceId} → ${ruta_rel}`);
    res.json({ ok:true });
...
router.post("/historial/:id/restaurar", async (req, res) => {
  try {
    ...
    console.log(`[PilotoAP] Restaurado: ${hist.ruta_rel} → ${fecha}`);
    ...
    for (const slug of [...new Set(slugs)]) {
      try {
        ...
        docs.forEach(d => {
          ...
          vehiculos.push({
            _id:        d._id,
            nombre:     extraerNombreVehiculo(raw, d.nombre),
...
router.get("/mapa", async (req, res) => {
  try {
    ...
    console.log("[PilotoAP/mapa] slugs a consultar:", slugs);

    const lotesParsed = [];

    for (const slug of slugs) {
      try {
        const docs = await _findAll(db.getDB(slug), { tipo:"aog_archivo" });
        console.log(`[PilotoAP/mapa] ${slug}: ${docs.length} docs`);
        ...
        console.log(`[PilotoAP/mapa] ${slug}: lotes encontrados:`, Object.keys(grupos));

        for (const [nombre, loteDocs] of Object.entries(grupos)) {
          const parsed = parseLote(loteDocs);
          console.log(`[PilotoAP/mapa] lote "${nombre}": boundary=${!!parsed.boundary} origen=${!!parsed.origen} sections=${parsed.sections?.length||0}`);
      ...
      } catch(e) { console.error(`[PilotoAP/mapa] ${slug}:`, e.message); }
    }
