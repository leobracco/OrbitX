...
const PRODUCTOS = [
  ...
  // ToolX = switch de herramienta inalámbrico (ESP32, PGN 253 → PilotX).
  "ToolX",
  // PilotXAndroid = APK de la tablet (AndroidPilotXUpdateService lo baja y
  // lanza el instalador del sistema). Producto distinto de PilotX (ZIP Windows).
  "PilotXAndroid",
  // PilotXParche = parche diferencial de PilotX (~5 MB, solo lo que cambio).
  // Se aplica con instalar-pilotx.ps1, que rechaza el parche si la version
  // instalada no es la base esperada. Producto aparte de PilotX a proposito:
  // el catalogo guarda un archivo por version y el completo tiene que seguir
  // disponible para equipos que vienen de versiones viejas.
  "PilotXParche",
